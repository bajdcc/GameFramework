...
namespace clib {
    ...
    clexer_json::clexer_json(string_t str) : str(str) {
        length = (uint)str.length();
        assert(length > 0);
        initMap();
    }

    clexer_json::~clexer_json() = default;

#define DEFINE_LEXER_GETTER(t) \
LEX_T(t) clexer_json::get_##t() const \
{ \
    return bags._##t; \
}
...
#define DEFINE_LEXER_GETTER(t) \
LEX_T(t) clexer_json::get_store_##t(int index) const \
{ \
...
#undef DEFINE_LEXER_GETTER
        ...
        // 记录错误
        lexer_t clexer_json::record_error(error_t error, uint skip) {
        err_record_t err{};
        err.line = line; // 起始行
    ...
    const clexer_json::err_record_t& clexer_json::recent_error() const {
        return records.back();
    }

    lexer_t clexer_json::next() {
        auto c = local();
        if (c == -1) {
            type = l_end;
            return l_end;
    ...
    lexer_t clexer_json::get_type() const {
        return type;
    }

    int clexer_json::get_line() const {
        return line;
    }

    int clexer_json::get_column() const {
        return column;
    }

    int clexer_json::get_last_line() const {
        return last_line;
    }

    int clexer_json::get_last_column() const {
        return last_column;
    }

    string_t clexer_json::current() const {
        switch (type) {
        case l_operator:
    ...
    bool clexer_json::is_type(lexer_t type) const {
        return get_type() == type;
    }

    bool clexer_json::is_keyword(keyword_t type) const {
        return get_type() == l_keyword && get_keyword() == type;
    }

    bool clexer_json::is_operator(operator_t type) const {
        return get_type() == l_operator && get_operator() == type;
    }

    bool clexer_json::is_operator(operator_t type1, operator_t type2) const {
        return get_type() == l_operator && (get_operator() == type1 || get_operator() == type2);
    }

    bool clexer_json::is_number() const {
        return get_type() >= l_char && get_type() <= l_double;
    }

    bool clexer_json::is_integer() const {
        return get_type() >= l_char && get_type() <= l_ulong;
    }

    LEX_T(int) clexer_json::get_integer() const {
        assert(is_integer());
        switch (type) {
...
#undef DEFINE_LEXER_CASE
    ...
    void clexer_json::move(uint idx, int inc) {
        last_index = index;
        last_line = line;
        last_column = column;
        if (inc < 0) {
    ...
    // 数字类型后缀（带无符号）
    lexer_t clexer_json::digit_type(lexer_t t, uint& i) {
        if (i == length) {
            return l_error;
        }
        if (str[i] == 'U' || str[i] == 'u') {
    ...
    bool clexer_json::digit_from_integer(lexer_t t, LEX_T(ulong) n) {
        switch (t) {
...
#undef DEFINE_LEXER_CONV_INTEGER
    ...
    bool clexer_json::digit_from_double(lexer_t t, LEX_T(double) d) {
        switch (t) {
...
#undef DEFINE_LEXER_CONV_INTEGER
    ...
    // 返回数字（依照目前识别的类型）
    lexer_t clexer_json::digit_return(lexer_t t, LEX_T(ulong) n, LEX_T(double) d, uint i) {
        if (t == l_int) {
            bags._int = (int)n;
        }
        else if (t == l_uint) {
    ...
    lexer_t clexer_json::next_digit() {
        // 假定这里的数字规则是以0-9开头
        // 0x返回有符号，0X返回无符号
    ...
    lexer_t clexer_json::next_alpha() {
        uint i;
    ...
    lexer_t clexer_json::next_space() {
        uint i, j;
        switch (str[index]) {
        case ' ':
        case '\t':
    ...
    lexer_t clexer_json::next_string() {
        auto i = index;
        auto prev = str[i];
        // 寻找非'\"'的第一个'"'
    ...
    lexer_t clexer_json::next_comment() {
        uint i = index;
        if (str[++i] == '/') { // '//'
            // 寻找第一个换行符
    ...
    lexer_t clexer_json::next_operator() {
        auto c = local();
    ...
    int clexer_json::local() {
        if (index < length)
            return str[index];
        return -1;
    }

    int clexer_json::local(int offset) {
        if (index + offset < length)
            return str[index + offset];
        return -1;
    }

    void clexer_json::initMap() {
        // Keyword
