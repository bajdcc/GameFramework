                ...
                *p = '\0';
                ...
                if (exists(f)) {
                    ...
                    if (readfile("/dev/uuid", &puuid, &plen) == 0) {
                        ...
                        run(format("touch /fifo/ipc_res_%s", puuid));
                        run(format("mklink /ipc/req_%s /fifo/ipc_req_%s hide", puuid, puuid));
                        run(format("mklink /ipc/res_%s /fifo/ipc_res_%s hide", puuid, puuid));
                        ...
                        *p = ' ';
                        ...
                        run(format("rm /fifo/ipc_req_%s", puuid));
                        run(format("rm /fifo/ipc_res_%s", puuid));
                        free(puuid);
                    }
                }
                else {
