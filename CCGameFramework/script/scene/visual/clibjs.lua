...
local JS = require('script.lib.ui.clibjs')

local modname = 'script.scene.visual.clibjs'
local M = Scene:new()
_G[modname] = M
...
function M:init()
	...
	UIExt.trace('Scene [ClibJS Engine] init')
	-- INFO
	...
	UIExt.trace('Scene [ClibJS Engine]: create background #' .. self.layers.bg.handle)
	-- TEXT
	local cc = Text:new({
		color = '#222222',
		text = 'Made by bajdcc',
	...
	UIExt.trace('Scene [ClibJS Engine]: create text #' .. self.layers.cc.handle)
	-- TEXT
	local text = Text:new({
		color = '#222222',
		text = self.name,
	...
	UIExt.trace('Scene [ClibJS Engine]: create text #' .. self.layers.text.handle)
	-- MENU
	self:init_menu(info)
...
function M:destroy()
	UIExt.trace('Scene [ClibJS Engine] destroy')
	UIExt.clear_scene()
end

function M:init_event()
	self.handler[self.win_event.created] = function(this)
		UIExt.trace('Scene [ClibJS Engine] Test created message!')
	end
...
function M:init_menu(info)
	...
	self:add(bg)
	self.layers.pe2d = bg:add(JS:new({
		hit = function(this, evt)
